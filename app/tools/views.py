"""Structured views of forecast and anomaly results, shared by every consumer of the tool layer.

The MCP server (``app/mcp/schemas.py``) and the HTTP API (``app/api``) both present forecasts and
anomalies through these views, so the fields a consumer must never lose (model, cutoff, interval,
backtest, baseline; detector, expected value, score, severity, direction) are defined once. The
views only copy fields from the typed Phase 3 results: no number is recomputed here.
"""
